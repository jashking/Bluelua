...
#define LOCTEXT_NAMESPACE "CallLuaFunctionHandler"
 ...
 */
void FKCHandler_CallLuaFunction::CreateFunctionCallStatement(FKismetFunctionContext& Context, UEdGraphNode* Node, UEdGraphPin* SelfPin)
{
	...
	{
		UFunction* ReplaceFunction = FindReplaceFunction(Context, Node);
		...
		if (NumErrorsAtStart == CompilerContext.MessageLog.NumErrors)
		{
			...
			for (FBPTerminal* Target : ContextTerms)
			{
				...
				UFunction* Func = CallingContext->FindFunctionByName(TEXT("FakeLua"));
				//UFunction* NewFunction = DuplicateObject<UFunction>(Func, Func->GetOuter(), TEXT("TestLua"));

				UFunction* NewFunction = NewObject<UFunction>(ReplaceFunction->GetOuter(), TEXT("TestLua"), RF_Public);
				NewFunction->ReturnValueOffset = MAX_uint16;
				NewFunction->FirstPropertyToInit = NULL;
				NewFunction->FunctionFlags = Func->FunctionFlags;
				NewFunction->Children = Func->Children;
				NewFunction->StaticLink(true);
				NewFunction->SetNativeFunc(&ILuaImplementableInterface::MyProcessInternal1);

				//UFunction* NewFunction = DuplicateObject<UFunction>(Function, Function->GetOuter(), TEXT("TestLua"));
				//NewFunction->NumParms = NewFunction->NumParms + 1;
				//NewFunction->ParmsSize = 21;
				//NewFunction->FunctionFlags |= FUNC_Native;
				//NewFunction->SetNativeFunc(&ILuaImplementableInterface::MyProcessInternal1);

				//UNameProperty* NameProperty = NewObject<UNameProperty>(NewFunction->GetOuter(), NAME_None);
				//NameProperty->Next = NewFunction->Children;
				//NewFunction->Children = NameProperty;

				FBlueprintCompiledStatement& Statement = Context.AppendStatementForNode(Node);
				Statement.FunctionToCall = NewFunction/*Function*/;
				...
				UEdGraphPin* PinToTry = FEdGraphUtilities::GetNetFromPin(Node->FindPin(TEXT("FunctionName")));
				if (FBPTerminal** Term = Context.NetMap.Find(PinToTry))
				{
					FBPTerminal* RHSTerm = *Term;

					RHSTerms.Insert(RHSTerm, 0);
				}
...
bool FKCHandler_CallLuaFunction::IsCalledFunctionPure(UEdGraphNode* Node)
{
	if(UK2Node_CallLuaImplementation* CallFunctionNode = Cast<UK2Node_CallLuaImplementation>(Node))
	{
...
bool FKCHandler_CallLuaFunction::IsCalledFunctionFinal(UEdGraphNode* Node)
{
	if(UK2Node_CallLuaImplementation* CallFunctionNode = Cast<UK2Node_CallLuaImplementation>(Node))
	{
...
bool FKCHandler_CallLuaFunction::IsCalledFunctionFromInterface(UEdGraphNode* Node)
{
	if (UK2Node_CallLuaImplementation* CallFunctionNode = Cast<UK2Node_CallLuaImplementation>(Node))
	{
...
UClass* FKCHandler_CallLuaFunction::GetCallingContext(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
...
UClass* FKCHandler_CallLuaFunction::GetTrueCallingClass(FKismetFunctionContext& Context, UEdGraphPin* SelfPin)
{
	if (SelfPin)
	{
...
void FKCHandler_CallLuaFunction::RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
	if (UFunction* Function = FindFunction(Context, Node))
	{
...
void FKCHandler_CallLuaFunction::RegisterNet(FKismetFunctionContext& Context, UEdGraphPin* Net)
{
...
UFunction* FKCHandler_CallLuaFunction::FindFunction(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
	UClass* CallingContext = GetCallingContext(Context, Node);
...
UFunction* FKCHandler_CallLuaFunction::FindReplaceFunction(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
	UClass* CallingContext = GetCallingContext(Context, Node);

	if (CallingContext && CallingContext->ImplementsInterface(ULuaImplementableInterface::StaticClass()))
	{
		return CallingContext->FindFunctionByName(TEXT("TestLua"));
	}

	return nullptr;
}

void FKCHandler_CallLuaFunction::Transform(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
...
void FKCHandler_CallLuaFunction::Compile(FKismetFunctionContext& Context, UEdGraphNode* Node)
{
	check(NULL != Node);
...
void FKCHandler_CallLuaFunction::CheckIfFunctionIsCallable(UFunction* Function, FKismetFunctionContext& Context, UEdGraphNode* Node)
{
...
FString FKCHandler_CallLuaFunction::GetFunctionNameFromNode(UEdGraphNode* Node) const
{
	UK2Node_CallFunction* CallFuncNode = Cast<UK2Node_CallFunction>(Node);
	if (CallFuncNode)
	{
